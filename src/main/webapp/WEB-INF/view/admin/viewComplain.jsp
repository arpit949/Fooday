...
<script type="text/javascript">
	function reply(x) {
		var spx = document.getElementById("div");
		var id = document.getElementById("id");
		var loginid = document.getElementById("loginId");
		var complainDate = document.getElementById("complainDate");
		...
		htp.onreadystatechange = function() {
			if (htp.readyState == 4) {
				...
				for (var i = 0; i < jsn.length; i++) {
					var data = jsn[i];
					id.value = data.complainId;
					loginid = data.loginId;
					complainDate = data.complainDate;
					subject.value = data.complainName;
					description.value = data.complainDescription;
				}
			}
			$('#complainAdmin').modal('toggle');
		}
		htp.open("get", "complainDataGet.html?id=" + x, true);
		htp.send();
...
<body class="light">
	...
	<section class="content">
											...
											<tr>
												...
												<f:choose>
													<f:when test="${status eq 'pending'}">
														<td><button type="button" class="btn btn-danger btn-border-radius waves-effect" onclick='reply("${i.id}")'>Pending</button></td>
													</f:when>
													<f:otherwise>
		...
		<div class="modal fade" id="complainAdmin" role="dialog">
			<div class="modal-dialog">
				...
				<div class="modal-content">
					...
					<form:form action="replyComplainData.html" method="POST"
						modelAttribute="complainvo" id="form">

						<div class="form-group form-float">
							<form:input type="hidden" path="id" id="id" value="" />

							<input type="hidden" path="loginvo.loginId" value=""
						...
						<div class="form-group form-float active">
							<div class="form-line focused active">
								...
								<form:input type="text" path="complainDescription"
									id="description" value="" class="form-control" readonly="true" />
							</div>
						</div>
						<div class="form-group form-float active">
							<div class="form-line focused active">
								<label class="form-label active">Reply</label>
								<form:textarea rows="" cols="" path="reply" name="reply" id="reply"
									class="form-control" />
							</div>
						</div>
