...
<body class="light">
				...
				<div class="row">
					<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
						<ul class="breadcrumb breadcrumb-style ">
							...
							<li class="breadcrumb-item active">Restaurant Data</li>
						</ul>
					</div>
				</div>
			</div>
						...
						<div class="body">
							<div class="table-responsive">
								<table
									...
									<tbody>
										<f:forEach items="${data}" var="i">
											<tr>
												<td></td>
												<td><f:forEach items="${fn:split(i.profileImage,',')}"
														var="img">
														<img alt="" src="${img}"
												...
												<td>${i.gstNumber}</td>
												<td>${i.restaurantAddress}+${i.restaurantAddressPincode}</td>
												<td>${i.areavo.areaName}</td>
												<td>${i.cityvo.cityName}</td>
												<td>${i.statevo.stateName}</td>
												<td>${i.paymentMethod}</td>
												<td><f:set var="status" value="${i.loginvo.status}" />
													<f:if test="${status==false}">
														<button type="button"
															onclick="active(${i.loginvo.loginId})"
															class="btn btn-primary btn-border-radius waves-effect">DeActived</button>
													</f:if> <f:if test="${status==true}">
														<button type="button"
															onclick="deactive(${i.loginvo.loginId})"
	...
	<script type="text/javascript">
		...
		function active(x)
		{
			...
			htp.open("get","activateEmail.html?loginId="+x,true);
			htp.send();
		...
		function deactive(x)
		{
			...
			htp.open("get","deactivateEmail.html?loginId="+x,true);
			htp.send();
		...
		{
			alert("In redirect");
			window.location.href="viewRestaurant.html";
		}
	</script>
</body>
